...
function validateRawConfig(input: unknown, errors: ErrorSink): void {
  ...
  if (Object.hasOwn(input, "settings")) {
    ...
    if (rejectUnknownKeys(input.settings, allowed, "settings", errors)) {
      ...
      for (const key of ["hotReload", "worldRules", "adminBypass", "refundDeniedSpheres", "disableCaptureAim", "debugLogging"]) {
...
function compactSettings(settings: RuntimeSettingsValue): JsonRecord {
  return {
    ...
    ...(settings.disableCaptureAim ? { disableCaptureAim: true } : {}),
    ...
    if (Array.isArray(document.modes)) {
      document.modes.forEach((mode, index) => {
        ...
        if (!Object.hasOwn(mode.actions, "capture")) {
          ...
          const captureValue = modeId === "safe" ? false : true;
          mode.actions["capture"] = captureValue;
          addMigrationFallback(report, {
            fromVersion: 7,
            toVersion: 8,
            path: `$.modes[${index}].actions.capture`,
            message: `Version 7 had no capture action; defaulted capture to ${captureValue ? "true" : "false in safe mode"}. Set capture explicitly to change capture permission.`
          });
        }
      });
    }
