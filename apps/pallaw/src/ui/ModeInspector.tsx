    ...
    <div class="dialog-tab-content">
      <Show when={tab() === "rules"}>
        <div class="rules-stack">
          ...
          <div class="section-card combat-matrix-card"><div class="section-card-header"><div><h3>Combat matrix</h3><p>Every source and target relationship is an explicit damage multiplier. Deny is 0 and Allow is 1.</p></div></div><div class="section-card-body"><CombatMatrix matrix={props.mode.combat} isMode={true} overrideFor={(source, target) => props.mode.combat[source]?.[target] ?? 0} onChange={(source, target, value) => { props.onChange({ type: "set-combat", source, target, value }); }} /></div></div>
        </div>
      </Show>
