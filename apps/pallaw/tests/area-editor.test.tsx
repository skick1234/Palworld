...
describe("AreaEditor", () => {
  ...
  test("presents modes as a wrapping badge selector", async () => {
    const change = vi.fn();
    const area = { name: "North", mode: "safe", enabled: true, map: "world", minimumLevel: null, polygon: [[0, 0], [10, 0], [0, 10]] as [number, number][], actions: {}, combat: [], messages: {} };
    render(() => <AreaEditor area={area} isRegion={true} modes={[{ id: "safe", name: "Safe", color: "#22C55E" }, { id: "pvp", name: "PvP", color: "#EF4444" }]} maps={[{ id: "world", label: "World" }]} effectiveActions={{}} effectiveCombat={{}} modeName="Safe" messages={{ enabled: true, actionNames: {} }} resolvedMessages={{}} overrideFor={() => "default"} onChange={change} />);
